...
comment on table local is 'Pontos de estoque: galpão EON Instalações, obras, etc.';
...
comment on column insumo.unidade is 'Unidade de medida: m, un, rolo, pc...';
comment on column insumo.estoque_min is 'Estoque de segurança (folga para imprevistos).';
...
comment on table tipo_kit is 'Receita genérica do kit (molde). O BOM pendura aqui via composicao.';

-- Composição do kit (BOM): cada tipo de kit consome X de cada insumo.
create table composicao (
  id           uuid primary key default gen_random_uuid(),
  tipo_kit_id  uuid not null references tipo_kit(id) on delete cascade,
...
comment on table composicao is 'Bill of Materials. quantidade decimal de propósito (ex.: 10,5 m de fio).';
...
-- ---------------------------------------------------------------------------
create table de_para_fornecedor (
  id                  uuid primary key default gen_random_uuid(),
...
create index idx_de_para_ean on de_para_fornecedor (codigo_ean) where codigo_ean is not null;
create index idx_de_para_insumo on de_para_fornecedor (insumo_id);
...
create table lote (
  id                uuid primary key default gen_random_uuid(),
  tipo_kit_id       uuid not null references tipo_kit(id) on delete restrict,
  empreendimento_id uuid references empreendimento(id) on delete set null,
  quantidade        integer not null check (quantidade > 0),
  data_producao     date not null default current_date,
  created_at        timestamptz not null default now()
);

comment on table lote is 'Amarra a produção ao empreendimento (sem descer a bloco/apartamento).';

-- Unidade do kit: cada kit físico, com número e QR único.
create table unidade_kit (
  id          uuid primary key default gen_random_uuid(),
...
create index idx_unidade_kit_lote on unidade_kit (lote_id);
create index idx_unidade_kit_status on unidade_kit (status);

-- Sequência para o número legível global do kit (1, 2, 3... atravessando lotes).
create sequence if not exists seq_numero_kit start 1;
...
create table movimentacao (
  ...
  unidade_kit_id  uuid references unidade_kit(id) on delete restrict,
...
create index idx_mov_insumo on movimentacao (insumo_id);
create index idx_mov_unidade on movimentacao (unidade_kit_id);
create index idx_mov_data on movimentacao (data);

comment on table movimentacao is 'Livro-razão. Saldo de insumo = soma das movimentações. Base do controle anti-furto.';
...
create view saldo_insumo with (security_invoker = true) as
select
  i.id            as insumo_id,
  i.nome,
  i.unidade,
...
from insumo i
left join movimentacao m on m.insumo_id = i.id
group by i.id;

comment on view saldo_insumo is 'Saldo corrente por insumo (soma do livro-razão).';
...
alter table tipo_kit           enable row level security;
alter table composicao         enable row level security;
alter table de_para_fornecedor enable row level security;
alter table lote               enable row level security;
alter table unidade_kit        enable row level security;
